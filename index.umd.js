(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.SessionSDK = {}));
   ...
   */
  function shouldRecord(samplingRate = 1) {
      return getRandomNumber() <= samplingRate;
  }
  ...
  class ConsoleLogger {
      ...
      captureLog(level, args) {
          ...
          if (this.callback) {
              try {
                  ...
                  console.error('Session SDK: Error in console log callback:', error);
              }
          }
      }
      /**
      ...
      getStackTrace() {
          try {
              ...
              if (!stack)
                  return undefined;
              // Filter out SDK-related stack frames
              const lines = stack
                  .split('\n')
                  .filter(line => !line.includes('console-logger') &&
                  !line.includes('SessionSDK') &&
                  !line.includes('session-sdk'))
                  .slice(1, 6); // Limit to 5 lines
              return lines.join('\n');
          }
          catch (_a) {
              return undefined;
  ...
  class NetworkLogger {
      ...
      patchFetch() {
          ...
          window.fetch = async function (input, init) {
              ...
              const networkRequest = {
                  method,
                  url,
                  requestHeaders: self.extractHeaders(request.headers),
                  requestBody: await self.extractRequestBody(request),
                  timestamp: startTime,
                  duration: 0,
              };
              self.requestMap.set(request, networkRequest);
              try {
                  const response = await self.originalFetch(request);
                  const endTime = Date.now();
                  networkRequest.status = response.status;
                  networkRequest.statusText = response.statusText;
                  networkRequest.responseHeaders = self.extractHeaders(response.headers);
                  networkRequest.duration = endTime - startTime;
                  // Try to extract response body (clone response to avoid consuming it)
                  try {
                      const clonedResponse = response.clone();
                      networkRequest.responseBody = await self.extractResponseBody(clonedResponse);
                  }
                  catch (_a) {
                      // Ignore response body extraction errors
                  }
                  self.captureRequest(networkRequest);
                  return response;
              }
              catch (error) {
                  const endTime = Date.now();
                  networkRequest.duration = endTime - startTime;
                  networkRequest.error = error instanceof Error ? error.message : String(error);
                  self.captureRequest(networkRequest);
                  throw error;
              }
          };
      ...
      patchXHR() {
          ...
          XMLHttpRequest.prototype.open = function (method, url, async, username, password) {
              const extendedXhr = this;
              extendedXhr._sessionSDKMethod = method;
              extendedXhr._sessionSDKUrl = url.toString();
              extendedXhr._sessionSDKStartTime = Date.now();
              return self.originalXHROpen.call(this, method, url, async || true, username, password);
          };
          XMLHttpRequest.prototype.send = function (body) {
              const xhr = this;
              const method = xhr._sessionSDKMethod || '';
              const url = xhr._sessionSDKUrl || '';
              const startTime = xhr._sessionSDKStartTime || Date.now();
              const networkRequest = {
                  method,
                  url,
                  requestBody: body,
              ...
              self.requestMap.set(xhr, networkRequest);
              // Capture response
              const originalOnReadyStateChange = xhr.onreadystatechange;
              xhr.onreadystatechange = function () {
                  if (xhr.readyState === XMLHttpRequest.DONE) {
                      const endTime = Date.now();
                      const request = self.requestMap.get(xhr);
                      if (request) {
                          request.status = xhr.status;
                          request.statusText = xhr.statusText;
                          request.responseHeaders = self.extractXHRHeaders(xhr);
      ...
      captureRequest(request) {
          ...
          if (this.callback) {
              try {
                  ...
                  // Silently fail to avoid breaking network requests
                  console.error('Session SDK: Error in network request callback:', error);
              }
          }
      }
      /**
  ...
  class UploadQueue {
      ...
      add(sessionData) {
          ...
          // Process immediately if this is the first item
      ...
      async processQueue(force = false) {
          if (this.isProcessing || (!force && this.queue.length === 0)) {
              return;
          }
          ...
          try {
              ...
              console.error('Session SDK: Error processing upload queue:', error);
          }
          finally {
              this.isProcessing = false;
          }
      ...
      async uploadItem(item) {
          try {
              const success = await this.uploadData(item.data);
              ...
              console.error('Session SDK: Error uploading session data:', error);
       ...
       */
      async uploadData(sessionData) {
          return new Promise((resolve) => {
              try {
                  // Use XMLHttpRequest to avoid conflicts with network logger's fetch patch
                  ...
                  xhr.onerror = () => {
                      console.error('Session SDK: Upload failed: XHR error');
                      resolve(false);
                  };
                  xhr.ontimeout = () => {
                      console.error('Session SDK: Upload failed: XHR timeout');
                      resolve(false);
                  };
                  ...
                  console.error('Session SDK: Upload failed:', error);
  ...
  class SessionSDK {
      constructor(apiKey, options = {}) {
          ...
          // Create upload queue BEFORE starting network logging to get original fetch
          this.uploadQueue = new UploadQueue(this.options.endpoint, this.apiKey, this.options.headers);
      }
      /**
      ...
      init() {
          if (!isSupported()) {
              console.warn('Session SDK: Browser not supported');
              return;
          }
          if (this.options.respectDNT && isDNTEnabled()) {
              return;
          }
          if (!shouldRecord(this.options.samplingRate)) {
              return;
          }
          this.startRecording();
      }
      ...
      startRecording() {
          ...
          this.events = [];
          // Start recording with a longer delay to ensure React has fully rendered
          setTimeout(() => {
              // Additional check to ensure React has rendered
              ...
              this.startRrwebRecording();
          }, 3000); // Wait 3 seconds for React to render completely
          // Start console logging
          if (this.options.captureConsole) {
              this.consoleLogger.start((log) => {
                  // Console logs are collected but not immediately uploaded
                  // They'll be included in the final session data
              });
          }
          // Start network logging
          if (this.options.captureNetwork) {
              this.networkLogger.start((request) => {
                  // Network requests are collected but not immediately uploaded
                  // They'll be included in the final session data
              });
          }
      }
      ...
      mergeOptions(options) {
          return {
              ...
              uploadInterval: 10 * 1000, // 10 seconds
              respectDNT: true,
              headers: {},
              ...options,
          };
      ...
      scheduleUpload() {
          ...
          // Upload every uploadInterval
          setInterval(uploadData, this.options.uploadInterval);
          // Also upload when session duration exceeds max
          setTimeout(() => {
              if (this.isRecording) {
                  this.stop();
              }
      ...
      uploadSessionData() {
          ...
          const hasConsoleLogs = this.consoleLogger.getLogs().length > 0;
          const hasNetworkRequests = this.networkLogger.getRequests().length > 0;
          if (!hasEvents && !hasConsoleLogs && !hasNetworkRequests) {
              return; // Don't upload empty sessions
          }
          const sessionData = {
              sessionId: this.sessionId,
              projectId: this.options.projectId || undefined, // Only include if set
              userId: this.options.userId || undefined,
              attributes: this.options.attributes,
              metadata: collectMetadata(),
              events: [...this.events],
              consoleLogs: this.consoleLogger.getLogs(),
              networkRequests: this.networkLogger.getRequests(),
              timestamp: this.startTime,
              duration: this.getSessionDuration(),
          };
          this.uploadQueue.add(sessionData);
          // Clear events after upload to prevent memory buildup
          this.events = [];
      }
      /**
  ...
  function init(apiKey, options = {}) {
      if (globalInstance) {
          console.warn('Session SDK: Already initialized, destroying previous instance');
          globalInstance.destroy();
      }
      globalInstance = new SessionSDK(apiKey, options);
      globalInstance.init();
  ...
  function setAttributes(attributes) {
      if (globalInstance) {
          globalInstance.setAttributes(attributes);
      }
  }
  /**
  ...
  function getSessionId() {
      return globalInstance ? globalInstance.getSessionId() : null;
  }
  /**
